...
class HUD(object):
    ...
    def render(self, display):
        if self._show_info:
            # Create a surface for the HUD info
            info_surface = pygame.Surface((220, self.dim[1]))  # Full height, fixed width
            info_surface.set_alpha(100)  # Semi-transparent
            display.blit(info_surface, (0, 0))  # Draw the HUD surface at the top-left corner
            ...
            for item in self._info_text:
                ...
                if isinstance(item, list):
                    if len(item) > 1:
                        # Draw collision history as lines
                        points = [(x + 8, v_offset + 8 + (1.0 - y) * 30) for x, y in enumerate(item)]
                        pygame.draw.lines(display, (255, 136, 0), False, points, 2)
                    item = None
                    v_offset += 18  # Move to the next line

                elif isinstance(item, tuple):
                    if isinstance(item[1], bool):
                        # Draw boolean indicators (e.g., Throttle, Brake)
                ...
                if item:  # Render the string
                    surface = self._font_mono.render(item, True, (255, 255, 255))
                    display.blit(surface, (8, v_offset))
                v_offset += 18  # Move to the next line

        # Render notifications and help text
...
def main():
    ...
    print(str(args.width) +"  " + str(args.height))
